Shader "CelShading/Complete/RampCelShaded"
{
    Properties
    {
    ...
    SubShader
    {
		...
		float4 LightingCel(SurfaceOutput s, half3 lightDir, half3 viewDir, half atten)
		{
			...
			float lightingAmount = (diffuse + specular + rimSmooth);
			lightingAmount = saturate(lightingAmount);
			float lightingRamp = tex2D(_LightingRamp, float2(lightingAmount * 0.5 - 0.5, 0.5));

			float3 col = s.Albedo * (lightingRamp * _LightColor0 + unity_AmbientSky);
